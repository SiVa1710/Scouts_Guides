...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _requestInterstitialAd(int index) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    DateTime? lastAdShown = prefs.getString('lastAdShown') != null
        ? DateTime.parse(prefs.getString('lastAdShown')!)
        : null;
    DateTime currentTime = DateTime.now();

    if (lastAdShown == null || currentTime.difference(lastAdShown).inMinutes >= 5) {
      if (_isAdLoaded && !_adRequested) {
        _interstitialAd?.fullScreenContentCallback = FullScreenContentCallback(
          onAdDismissedFullScreenContent: (InterstitialAd ad) {
            // When the ad is dismissed, navigate to the page
            navigateToCategory(context, index);
            prefs.setString('lastAdShown', currentTime.toString());
            _adRequested = false;
          },
          onAdFailedToShowFullScreenContent: (InterstitialAd ad, AdError error) {
            // Failed to show the ad, navigate to the page
            navigateToCategory(context, index);
            _adRequested = false;
          },
        );
